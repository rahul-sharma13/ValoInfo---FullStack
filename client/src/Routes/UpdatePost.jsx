...
const UpdatePost = () => {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get(`http://localhost:8000/api/v1/post/getposts?postId=${params.postId}`)
            .then((res) => {
    ...
    const handleUpdateSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            await axios.patch(`http://localhost:8000/api/v1/post/update/${params.postId}/${currentUser._id}`, formData, { withCredentials: true, credentials: 'include' }).then((res) => {
                setLoading(false);
                setTimeout(() => {
