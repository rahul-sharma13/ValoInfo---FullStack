...
const Discussions = () => {
    ...
    const { data: postOnTopic, isLoading: onTopicLoad, isError: topicError } = useQuery({
        ...
        queryFn: async () => {
            if (topic !== '') {
                const res = await axios.get(`${import.meta.env.VITE_LOCAL_BASE_URL}/post/getPosts?topic=${topic}`);
                return res.data;
            }
            return [];
        }
    ...
    const { data: posts, isLoading: postsLoad, isError: postsError } = useQuery({
        ...
        queryFn: async () => {
            const res = await axios.get(`${import.meta.env.VITE_LOCAL_BASE_URL}/post/getAllPosts?inTime=${inTime}&order=${order}`);
            return res.data;
        }
    })
    ...
    const { data: searchedPosts, isLoading: searchedLoading, isError: searchedError } = useQuery({
        ...
        queryFn: async () => {
            const res = await axios.get(`${import.meta.env.VITE_LOCAL_BASE_URL}/post/getPosts?searchTerm=${debouncedSearchTerm}`);
            return res.data;
        },
