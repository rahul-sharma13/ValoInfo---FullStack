...
const Discussions = () => {
  ...
  useEffect(() => {
    const getTopicPosts = async () => {
      try {
        await axios.get(`https://valo-info-api.vercel.app/api/v1/post/getPosts?topic=${topic}`).then((res) => {
  ...
  useEffect(() => {
    const getPosts = async () => {
      setLoading(true);
      try {
        await axios.get(`https://valo-info-api.vercel.app/api/v1/post/getAllPosts?inTime=${inTime}&order=${order}`).then((res) => {
          setLoading(false);
  ...
  const handleKeyPress = (event) => {
    if (event.key === 'Enter') {
      const getSearchedPosts = async () => {
        await axios.get(`https://valo-info-api.vercel.app/api/v1/post/getPosts?searchTerm=${searchTerm}`).then((res) => {
          setLoading(false);
          // console.log(res);
          setPosts(res?.data?.data?.posts);
          setSearchTerm('');
