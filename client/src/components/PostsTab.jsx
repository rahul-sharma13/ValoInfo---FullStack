...
const PostsTab = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        setLoading(true);
        await axios.get(`http://localhost:8000/api/v1/post/getuserposts/${currentUser._id}`, { withCredentials: true, credentials: 'include' }).then((res) => {
          // console.log(res);
          setLoading(false);
  ...
  const handleShowMore = async () => {
    ...
    try {
      await axios.get(`http://localhost:8000/api/v1/post/getuserposts/${currentUser._id}?startIndex=${startIndex}`, { withCredentials: true, credentials: 'include' }).then((res) => {
  ...
  const handleDeletePost = async (id) => {
    try {
      await axios.delete(`http://localhost:8000/api/v1/post/delete/${id}`, { withCredentials: true, credentials: "include" }).then((res) => {
        console.log(res);
